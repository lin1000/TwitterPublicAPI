...
def check_single_tweet(tweetid):
 
    datafiles = "../python/05GNIPData/*.json.gz"
    outputfilepath = "../spark/08GNIPDataFilterByTweetID/"

    dataRDD = sc.textFile(datafiles).map(lambda x : json.loads(x))

    print "Loaded %s json records" % (dataRDD.count())

    filteredResult = dataRDD.filter(lambda t: "body" in t).filter(lambda t: t['id']==tweetid).map(lambda t: gnip_2_csv(t))
        
    #save filtered result into files
    #filteredResult.saveAsTextFile(outputfilepath + "/" + tweetid)

if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--tweetid", dest='tweetid',default=None, help="single tweetid to be extrcted")
    args = parser.parse_args()
    if(args.tweetid is None):
        print "tweet id is not specified, do nothing and exit."
        parser.print_help()
        sys.exit(1)
    else:
        print "tweet id is {}".format(args.tweetid)
 
    conf = SparkConf().setAppName("Read entire json activities and Check on a single tweet")
    sc = SparkContext(conf=conf)
    sqlContext = SQLContext(sc)
    check_single_tweet(args.tweetid)
    sc.stop()
